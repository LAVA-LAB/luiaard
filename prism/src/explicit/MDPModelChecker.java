...
public class MDPModelChecker extends ProbModelChecker
{
	...
	protected StateValues checkProbPathFormulaLTL(Model<?> model, Expression expr, boolean qual, MinMax minMax, BitSet statesOfInterest) throws PrismException
	{
		...
		product = mcLtl.constructProductMDP(this, (MDP<Double>) model, expr, statesOfInterest, allowedAcceptance);
	...
	{
		LTLModelChecker mcLtl;
		MDPRewards<Double> productRewards;
		...
		product = mcLtl.constructProductModel(da, (MDP<Double>)model, labelBS, statesOfInterest);
		timer.stop("product has " + product.getProductModel().infoString());

		// Adapt reward info to product model
		productRewards = ((MDPRewards<Double>) modelRewards).liftFromModel(product);
