...
public class LTLModelChecker extends PrismComponent
{
	 ...
	 public <Value,M extends Model<Value>> LTLProduct<M> constructProductModel(DA<BitSet,? extends AcceptanceOmega> da, M model, Vector<BitSet> labelBS, BitSet statesOfInterest) throws PrismException
	{
		...
		case STPG: {
			STPGExplicit<Value> stpgProd = new STPGExplicit<>();
		...
		for (int s_0 : new IterableStateSet(statesOfInterest, model.getNumStates())) {
			...
			switch (modelType) {
			case STPG:
				((STPGExplicit<Value>) prodModel).addState(((STPG<Value>) model).getPlayer(s_0));
				break;
			default:
				prodModel.addState();
			break;
			...
			for (int j = 0; j < numChoices; j++) {
				...
				while (iter.hasNext()) {
					...
					if (!visited.get(s_2 * daSize + q_2) && map[s_2 * daSize + q_2] == -1) {
						...
						switch (modelType) {
						case STPG:
							((STPGExplicit<Value>) prodModel).addState(((STPG<Value>) model).getPlayer(s_2));
							break;
						default:
							prodModel.addState();
							break;
				...
				case MDP:
					...
					break;
				case STPG:
					((STPGExplicit<Value>) prodModel).addActionLabelledChoice(map[s_1 * daSize + q_1], prodDistr, ((STPG<Value>) model).getAction(s_1, j));
					break;
				default:
					break;
				}
