...
public class SimulatorEngine extends PrismComponent
{
	...
	private HashMap<State,Integer> reachableStatesMap;
	// Strategy
	...
	/**
	 * Constructor for the simulator engine with fixed seed.
	 */
	public SimulatorEngine(PrismComponent parent, int seed)
	{
		super(parent);
		modelGen = null;
		modelType = null;
	...
	{
		return automaticTransition(null);
	}
	 ...
	 * Otherwise, the function returns true indicating that a transition was successfully taken. 
	 */
	public boolean automaticTransition(HashSet<Integer> target) throws PrismException
	{
		...
		if (target != null) {
			if (target.contains(getReachableStateIndex(currentState))) {
				return false;
			}
		}
		...
		case DTMC:
			...
			break;
		case MDP:
			if (strategy != null && strategy instanceof MRStrategy) {
				int s = getReachableStateIndex(currentState);
				i = ((MRStrategy) strategy).getChoice(s, rng);
			} else if (strategy != null && strategy instanceof MDStrategy) {
				int s = getReachableStateIndex(currentState);
				i = ((MDStrategy) strategy).getChoiceIndex(s);
			} else {
	 ...
	 * Otherwise, the function returns true indicating that a transition was successfully taken. 
	 */
	public boolean automaticTransitionByChoice(int choice, HashSet<Integer> target) throws PrismException
	{
	 ...
	 * The function returns the number of transitions successfully taken. 
	 */
	public int automaticTransitions(int n, boolean stopOnLoops, HashSet<Integer> target) throws PrismException
	{
		int i = 0;
		while (i < n && !(stopOnLoops && path.isLooping())) {
			if (!automaticTransition(target))
				break;
			i++;
		}
		return i;
	...
	public void loadReachableStates(List<State> reachableStates)
	{
		...
		reachableStatesMap = new HashMap<>();
		int numStates = reachableStates.size();
		for (int s = 0; s < numStates; s++) {
			reachableStatesMap.put(reachableStates.get(s), s);
		}
	}
	...
	private void initialiseStrategy()
	{
		if (strategy != null) {
			...
			int s = getReachableStateIndex(state);
	...
	public int getReachableStateIndex(State state) {
		if (reachableStatesMap != null) {
			return reachableStatesMap.get(state);
		} else if (reachableStates != null) {
			return reachableStates.indexOf(state);
		} else {
			return -1;
		}
	}
