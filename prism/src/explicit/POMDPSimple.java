...
public class POMDPSimple<Value> extends MDPSimple<Value> implements POMDP<Value>
{
	...
	@Override
	public double getCostAfterAction(Belief belief, int action, MDPRewards<Double> mdpRewards)
	{
	...
	@Override
	public double getCostAfterAction(double[] beliefInDist, int action, MDPRewards<Double> mdpRewards)
	{
		double cost = 0;
	...
	public double[] getBeliefInDistAfterAction(double[] beliefInDist, int action)
	{
		...
		for (int sp = 0; sp < n; sp++) {
			if (beliefInDist[sp] >= 1.0e-6) {
				...
				Distribution<Double> distr = (Distribution<Double>) getChoice(sp, action);
